...
def run():
    ...
    with torch.no_grad():
        ...
        for i, data in tqdm(tqdm(enumerate(data_loader))):
            ...
            text_embed, vid_embed, video_token = model(data, return_embeds=True)
            video_token = video_token.reshape(vid_embed.size()[0], 4, -1, 256).mean(dim=2)

            text_embed_arr.append(text_embed.cpu().detach())
            vid_embed_arr.append(vid_embed.cpu().detach())

            video_token_arr.append(video_token.cpu().detach())
            # text_token_arr.append(text_token.cpu().detach())

            # sim1 += sim_matrix_token2token(text_token, video_token)
    text_embeds = torch.cat(text_embed_arr)
    vid_embeds = torch.cat(vid_embed_arr)

    video_token = torch.cat(video_token_arr)
    # text_token = torch.cat(text_token_arr)
    mask = None
    if data_loader.dataset.sliding_window_stride != -1:
        cpu_vid_embeds = vid_embeds
        cpu_text_embeds = text_embeds
        cpu_video_token = video_token
        # cpu_text_token = text_token
        ...
        li_token_embeds = [x for x in cpu_video_token]
        # li_texttoken_embeds = [x for x in cpu_text_token]
        ...
        #                        'captions': raw_caps})
        vid_df = pd.DataFrame({'videoid': videoids, 'vid_embed': li_vid_embeds, 'txt_embed': li_txt_embeds,
                               'token_embed':li_token_embeds,  'captions': raw_caps})
        ...
        for vid in vid_df['videoid'].unique():
            ...
            token = torch.stack((tdf['token_embed'].values.tolist()))
    ...
    if args.split != 'train':  # because train is usually too big
        sims = sim_matrix(text_embeds, vid_embeds) + sim_matrix_token(text_embeds, video_token)
        sims = sims.numpy()

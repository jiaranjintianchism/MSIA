...
# from test.mmdetection import test
...
class TextVideoDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        try:
            if os.path.isfile(video_fp):
                imgs, idxs = self.video_reader(video_fp, self.video_params['num_frames'], frame_sample,
                                               fix_start=fix_start)
                # img = imgs[1]
                # object = test.get_object()
                # img = img.cpu().numpy().permute(2, 1, 0)
                # object_value = object.out(img=img)
                # region = []
                # for i in range(len(object_value)):
                #     region.append(object_value[str(i)][np.newaxis, :])
                # img = np.concatenate(region, 0)
                # print()
        ...
        if self.transforms is not None:
            # img = imgs[1]
            # object = test.get_object()
            # img = img.cpu().numpy().transpose(2, 1, 0)
            # object_value = object.out(img=img)
            # img = object_value
            # region = []
            # length = []
            # len_ = 0
            # for i in range(len(object_value)):
            #     val = object_value[str(i)][np.newaxis, :]
...
def read_frames_decord(video_path, num_frames, sample='rand', fix_start=None):
    ...
    a = frames.numpy()
    # name = video_path[-10:-4]
    # for i in range(16):
    #     a[i] = BGR_to_RGB(a[i])
    #     cv2.imwrite('/home/chenlei/Videos/mine/img/' + name + str(i) + '.png', a[i])

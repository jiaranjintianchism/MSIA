...
from model.model import sim_matrix, sim_matrix_token, sim_matrix_token2token
...
class Trainer(BaseTrainer):
        ...
        with tqdm(zip(*self.data_loader), desc=f"Training epoch {epoch}", total=total_iterations) as progress:
            for batch_idx, data_li in enumerate(progress):
                ...
                for dl_idx, data in enumerate(data_li):
                    ...
                    text_embeds, video_embeds, video_token = self.model(data)
                    b, embd = video_embeds.size()
                    video_token = video_token.reshape(b, 8, -1, 256).mean(dim=2)
                    output = sim_matrix(text_embeds, video_embeds)
                    output_token = sim_matrix_token(text_embeds, video_token)
                    # reg = sim_matrix(text_embeds, region)
                    # output_token2token = sim_matrix_token2token(text_token, video_token)
                    # output = self.model(data)
                    loss1 = self.loss(output)
                    loss2 = self.loss(output_token)
                    # loss3 = self.loss(output_token2token)
                    loss = loss1 + loss2
                    loss.backward()
                    self.optimizer.step()
    ...
    def _valid_epoch(self, epoch):
        ...
        video_token_arr = {x: [] for x in range(len(self.valid_data_loader))}
        text_token_arr = {x: [] for x in range(len(self.valid_data_loader))}
            ...
            for dl_idx, dl in enumerate(self.valid_data_loader):
                for data in tqdm(dl, desc=f"Validating dl{dl_idx}"):
                    ...
                    if avoid_data_parallel:
                        text_embed, vid_embed,  video_token = self.model.module(data, return_embeds=True)
                    else:
                        text_embed, vid_embed, video_token = self.model(data, return_embeds=True)

                    text_embed_arr[dl_idx].append(text_embed.cpu())
                    vid_embed_arr[dl_idx].append(vid_embed.cpu())

                    video_token_arr[dl_idx].append(video_token.cpu())
                    # text_token_arr[dl_idx].append(text_token.cpu())

                    sims_batch = sim_matrix(text_embed, vid_embed)
                    output_token = sim_matrix_token(text_embed, video_token)
                    output_token = output_token + output_token.t()
                    # sim_region = sim_matrix(text_embed, region)
                    # output_token2token = sim_matrix_token2token(text_token, video_token)
                    loss = self.loss(sims_batch) + self.loss(output_token)
        ...
        for dl_idx in range(len(self.valid_data_loader)):
            ...
            video_token = torch.cat(video_token_arr[dl_idx])
            sims = sim_matrix(text_embeds, vid_embeds).detach().cpu().numpy() + sim_matrix_token(text_embeds, video_token).detach().cpu().numpy()
